...
void OTA::pollWebserver()
{
  ...
  while (client.connected())
  {
    ...
    {
      // handlers in here must consume the rest of the body
      if (req_str.startsWith("GET /ping "))
        // On a ping send out confirmation and device information
        handlePing(client);
      break;
    }
    if (c == '\n' && currentLineIsBlank)
...
// Upon receiving something on /ping print out a confirmation and some device information
// this is used to detect the sensebox
void OTA::handlePing(WiFiClient client)
{
  LOG.println("Received Ping responding with device information");
  IPAddress iplocal = WiFi.localIP();
  String message = "Ping was successful!";
